...
const summary = [
  ...
  `- Verified candidates: **${report.totalDeals}** (${report.newDeals || 0} newly observed; not manual Slack approvals)`,
  `- Food candidates: ${report.foodDiscovery?.foodDeals || 0} (${report.foodDiscovery?.newFoodDeals || 0} newly observed)`,
  `- Food account share: ${report.foodDiscovery?.selectedFoodAccounts || 0}/${report.selectedAccounts?.length || 0}`,
